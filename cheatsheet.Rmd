...
Barchart with counts of each category in no particular order. i.e.
...
Barchart with counts with x-axis following the ordering. i.e.
...
**Use for:** Continuous variable without grouping. Interest in difference to H0.
    ...
    The H0 represents a single value, e.g. 100, wherease the H1 represents all other (possible) values
...
H1: Regressoin coefficient is not equal to 0
...
E.g.
- First regressoin: How much sales revenue would we expect if the store spends 50€ on advertisements:
...
- Causal relationship needs to be established theoretically:
    ...
    - Third variable causes both variables for which we meassure correlation -> high correlation without causality
